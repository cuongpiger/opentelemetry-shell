...
otel_trace_start_child_span() {
    ...
    trace_data="${otel_trace_resource_spans}"
    trace_data=$(otel_trace_add_resource_scopespans_span "$trace_data" \
        "$name" \
        "$OTEL_TRACE_ID" \
        "$span_id" \
        "$PARENT_SPAN_ID" \
    ...
    if [ -z "${OTEL_LOG_LEVEL-}" ]; then
        net_client_post "${trace_data}" "${OTEL_EXPORTER_OTEL_ENDPOINT}/v1/traces"
    else
        log_debug "[$(caller)] $*" >&2
        log_debug "BASH_SOURCE: ${BASH_SOURCE[*]}"
        log_debug "BASH_LINENO: ${BASH_LINENO[*]}"
